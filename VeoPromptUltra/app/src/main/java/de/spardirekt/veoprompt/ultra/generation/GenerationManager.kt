...
import de.spardirekt.veoprompt.ultra.worker.GenerationForegroundService
...
class GenerationManager(
    ...
    private suspend fun runPipeline(projectId: String, resume: Boolean) {
        ...
        val input = GenerationPipeline.PipelineInput(
            ...
            creativeMode = CreativeMode.fromRaw(project.creativeMode),
            ...
            onSuccess = { bundle ->
                ...
                repository.save(
                    current.copy(
                        ...
                        safetyAuditJson = json.encodeToString(bundle.safetyAudit),
                ...
                settingsStore.setLastPipelineStage(GenerationStage.DONE.name)
    ...
    private suspend fun markGenerating(projectId: String, stage: GenerationStage) {
        ...
        settingsStore.setLastPipelineStage(stage.name)
        _stage.value = stage
    }
    ...
    private suspend fun persistStage(projectId: String, update: GenerationPipeline.StageUpdate) {
        ...
        when (update.stage) {
            ...
            GenerationStage.CREATIVE_DIRECTOR -> {
                completed += GenerationStage.PHOTO_ANALYSIS
                completed += GenerationStage.PRODUCT_MODEL
                completed += GenerationStage.VISUAL_LOCK
            }
            GenerationStage.FINAL_PROMPT -> {
                completed += GenerationStage.PHOTO_ANALYSIS
                completed += GenerationStage.PRODUCT_MODEL
        ...
        repository.save(
            p.copy(
                ...
                veoPrompt = if (update.stage == GenerationStage.DONE) {
                    update.bundle?.veoPrompt ?: p.veoPrompt
                } else {
                    p.veoPrompt
                },
                voiceover = if (update.stage == GenerationStage.DONE) {
                    update.bundle?.voiceover ?: p.voiceover
                } else {
                    p.voiceover
                },
                title = if (update.stage == GenerationStage.DONE) {
                    update.bundle?.title ?: p.title
                } else {
                    p.title
                },
                ...
                safetyAuditJson = if (update.stage == GenerationStage.DONE) {
                    update.bundle?.safetyAudit?.let { json.encodeToString(it) } ?: p.safetyAuditJson
                } else {
                    p.safetyAuditJson
                },
        ...
        settingsStore.setLastPipelineStage(update.stage.name)
    }

    private suspend fun fail(projectId: String, error: AppError) {
        val p = repository.get(projectId) ?: return
        val safe = DebugLog.sanitize(error.detail)
        DebugLog.recordSafeError(safe)
        settingsStore.setLastSafeError(safe)
        settingsStore.setLastPipelineStage(GenerationStage.FAILED.name)
        repository.save(
            p.copy(
                status = ProjectStatus.Failed.name,
                generationStage = GenerationStage.FAILED.name,
                errorState = error.userMessage.ifBlank { "Не удалось завершить генерацию." },
                errorDetail = safe
            )
        )
