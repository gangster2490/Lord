...
import de.spardirekt.veoprompt.ultra.diagnostics.ErrorMapper
...
class OpenAiClient(
    ...
    fun clientFor(timeoutSeconds: Long): OkHttpClient {
        return OkHttpClient.Builder()
            .connectTimeout(ModelConfig.connectTimeoutSeconds(), TimeUnit.SECONDS)
    ...
    private fun executeChat(
            ...
            imageDataUrls.forEach { url ->
                add(buildJsonObject {
                    ...
                    put("image_url", buildJsonObject {
                        ...
                        put("detail", JsonPrimitive(ModelConfig.imageDetail(model)))
                    })
                })
            }
        }
        ...
        val sanitizedModel = ModelConfig.sanitize(model)
        val tokenLimit = ModelConfig.completionBudget(maxTokens)
        val requestBody = buildJsonObject {
            put("model", JsonPrimitive(sanitizedModel))
