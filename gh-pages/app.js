...
Gib exakt dieses JSON zurück:
{
  ...
  try {
    ...
    try {
      resp = await fetch(`${proxyUrl}/api/generate`, {
        ...
        headers: {
          ...
          'x-api-key-fwd':  key,        // proxy reads this, never stored
        },
      ...
      // Detect CORS vs connection refused
      const isCors = networkErr instanceof TypeError &&
        (networkErr.message.includes('fetch') || networkErr.message.includes('CORS') || networkErr.message === 'Failed to fetch');

      if (isCors) {
        throw new Error(
          `Proxy nicht erreichbar (<code>${proxyUrl}</code>).<br>` +
          `<b>Starte den Proxy lokal:</b><br>` +
          `<code>cd proxy && npm install && npm start</code><br>` +
    ...
    if (!resp.ok) {
      throw new Error(
        `Fehler ${resp.status}: ${data.error?.message || data.error || data.message || JSON.stringify(data)}`
      );
    }

    // Unwrap Anthropic envelope → extract JSON from content[0].text
