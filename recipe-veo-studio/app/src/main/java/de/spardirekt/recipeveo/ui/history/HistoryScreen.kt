...
fun HistoryScreen(
    ...
    LazyColumn(
        ...
        item {
            Text("История", style = MaterialTheme.typography.displayLarge)
            Spacer(Modifier.height(4.dp))
            Text("Готовые промпты и черновики с фото.", color = MaterialTheme.colorScheme.onSurfaceVariant)
        }
        if (rows.isEmpty()) {
            item { EmptyHint("Пока пусто", "Соберите первый промпт на вкладке «Создать».") }
        } else {
            items(rows, key = { it.id }) { project ->
                StudioCard(onClick = {
                    ...
                    MetaRow(project.status.label(), "${project.photos.size} фото")
