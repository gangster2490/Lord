...
data class StudioState(
    ...
    fun prefs(
        ...
        defaultVoice: VoiceLang = prefs.defaultVoice,
        tiktokShop: Boolean = prefs.tiktokShop,
    ) = copy(
        prefs = prefs.copy(
    ...
    fun library(): List<Project> = projects.filter {
        it.status != ProjectStatus.Draft || it.photos.isNotEmpty() || it.result != null
...
fun StudioState.ensureDraft(clock: AppClock): StudioState {
    val reusable = projects.firstOrNull {
        it.status == ProjectStatus.Draft && it.result == null
    }
