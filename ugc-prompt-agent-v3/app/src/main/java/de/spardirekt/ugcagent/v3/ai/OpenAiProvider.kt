...
class OpenAiProvider(
    ...
    private fun userContext(ctx: PromptContext): String = buildString {
        ...
        appendLine("PRODUCT IDENTITY LOCK (copy only this concise lock into the prompt; never dump the full fingerprint; never emit FINAL IDENTITY LOCK or DURATION):")
        appendLine(ctx.finalIdentityLock.ifBlank { "Keep exactly the same single physical product." })
        appendLine("Do not copy uncertain_hidden_geometry, ambiguity_warning, hidden mechanism assumptions, unconfirmed attachments, conflicting finish notes, listing/seller/marketplace dimensions, exact dimensions unless visually necessary and confidently verified, repeated Product Lock blocks, or long internal analysis into the final prompt.")
        ...
        appendLine("Choose setting, hook, lighting, speech and human behaviour from ONE evidence-based creative concept. Prefer the First Frame environment. Do not default every product to a kitchen. Do not invent a niche setting when context is uncertain.")
        appendLine("Lighting must match the setting. FORMAT, SETTING, ACTION, SPEECH and LIGHTING must describe one believable 8-second moment.")
