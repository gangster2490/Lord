...
class GeminiProvider(
    ...
    private fun userContext(ctx: PromptContext): String =
        "${ctx.firstFrameNote}\nStrict lock=${ctx.strictProductLock}\nSpeech=${ctx.speechLanguage}\nGenerator=${ctx.targetGenerator}\nPRODUCT IDENTITY LOCK:\n${ctx.finalIdentityLock}\nDo not copy uncertain_hidden_geometry, ambiguity_warning, listing dimensions or long internal analysis into the final prompt. Never emit FINAL IDENTITY LOCK, DURATION or STYLE. If color/finish conflict, SELECTED FIRST FRAME WINS.\nFingerprint (internal):\n${ctx.fingerprint}\nAction risk:\n${ctx.actionRisk}\nReadiness:\n${ctx.readiness}\nEvidence:\n${ctx.analysis}\nScene:\n${ctx.scene}\nPurchase appeal (internal, one desire only; do not dump this JSON into the prompt):\n${ctx.purchaseAppeal}\nUse all uploaded reference images as supporting identity evidence. Use only the selected LOW-RISK action. If motion_geometry_risk is HIGH, keep identity-critical moving components static. Choose setting, hook, lighting, speech and human behaviour from ONE evidence-based creative concept. Prefer the First Frame environment. Do not default every product to a kitchen. Do not invent a niche setting when context is uncertain. Lighting must match the setting. FORMAT, SETTING, ACTION, SPEECH and LIGHTING must describe one believable 8-second moment. Generate exactly 8.0 seconds. The spoken line must finish before the 8.0-second endpoint."
